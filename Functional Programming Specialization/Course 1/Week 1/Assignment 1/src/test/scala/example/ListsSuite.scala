...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum with negative number") {
    assert(sum(List(-2, 4, -1, 8)) == 9)
  }

  test("sum with zero and negative number") {
    assert(sum(List(0, 0, 0, 0, 2, -2)) == 0)
  }

  test("max with negative values") {
    assert(max(List(-1, -10, -23, -2)) == -1)
  }

  test("max with negative values and zeros") {
    assert(max(List(0, -2, 0, 1, -19, -30, 3, 15)) == 15)
  }
